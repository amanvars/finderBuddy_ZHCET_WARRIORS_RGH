...
class LanguageAdapter(LogicAdapter):
    ...
    def can_process(self, statement):
        ...
        """
        words = ['reorder','check','checking','checked','ordering','update','last','first','how','available','decrease','increment','reduce',
    ...
    def process(self, statement):
        ...
        if any(x in res for x in ['need','require','want']) and any(x in res for x in ['something','place']):
            print res
            response=Statement("Sure-go ahead and place your order")
            response.confidence=1
            return response

        if any(x in res for x in ['need','require','want']) and not any(x in res for x in ['how','cancel','change','modify','review','available','help','assist','increase','decrease','reduce','increment','something','place']):
            print "Need query"
            if(len(res)>2):
                ml=[]
                ml.append(res[2])
    ...
    #CHANGE ORDER TAGS
        ...
        elif any(x in res for x in ['review']) and not any(x in res for x in ['how']):
            print "Review query" 
            if(res[0]=='want' or res[0]=='need'):
                del(res[0])
            if not(len(res)==2 and (res[1].isdigit() or res[1]=='previous' or res[1]=='last' or res[1]=='first')):
        ...
        elif any(x in res for x in ['buy','best','find']) and not any(x in res for x in ['how','available','total','expect','expected']):
